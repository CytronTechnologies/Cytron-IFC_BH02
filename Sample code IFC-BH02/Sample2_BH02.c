...
//	Project description	: Sample2 for IFC-BH02 with IFC-CP04
...
void main(void)
{
	...
	while(1)
	{
		...
		//--------------------------------------------------------------------------------
		// This program is mainly for slave card,IFC-BH02. It will test the 4 push buttons
		...
		if(cp1_sw(1)==0)			// Test whether SW1 on IFC-CP04 is pressed
		{
			...
			for(i=0;i<1;i++)			// buzzer beeps and LCD displays mode for 1 time 
			{
				cp1_goto(0,5);			// set the position of message to be displayed
				cp1_str("Demo 1");		// send message to be displayed by calling function cp1_str
				buzzer = 1;				// ON buzzer
				delay(100000);			// time delay for buzzer ON
				cp1_clr();				// clear LCD screen by calling function cp1_clr
				buzzer = 0;				// OFF buzzer
				delay(50000);			// time delay for buzzer OFF
			}
			...
			while(1)
			{
				...
				cp1_str("Speed1:");			// send message to be displayed by calling function cp1_str
				cp1_dec(bh_1_spval(add_bh1),3);	// send decimal to be displayed by calling function cp1_dec
				...
				cp1_str("Speed2:");			// send message to be displayed by calling function cp1_str
				cp1_dec(bh_2_spval(add_bh1),3);	// send decimal to be displayed by calling function cp1_dec
		...
		else if(cp1_sw(2)==0)				// Test whether SW2 on IFC-CP04 is pressed
		{
			cp1_clr();					// clear LCD screen by calling function cp1_clr
			led2 = 1;					// turn ON LED2 on MB00 by sending value '1'

			for(i=0;i<2;i++)			// buzzer beeps and LCD displays mode for 2 time
			{
				cp1_goto(0,5);			// set the position of message to be displayed
				cp1_str("Demo 2");		// send message to be displayed by calling function cp1_str
				buzzer = 1;				// ON buzzer
				delay(100000);			// time delay for buzzer ON
				cp1_clr();				// clear LCD screen by calling function cp1_clr
				buzzer = 0;				// OFF buzzer
			...
			while(1)
			{
				...
				// and LCD displays the both speeds and encoders' value at the same time

				cp1_goto(0,0);					// set the position of message to be displayed
				cp1_str("Speed1:");				// send message to be displayed by calling function cp1_str
				cp1_dec(bh_1_spval(add_bh1),3);	// send decimal to be displayed by calling function cp1_dec
				cp1_str(" ");					// send message to be displayed by calling function cp1_str
				cp1_dec(bh_1_enval(add_bh1),5);	// send decimal to be displayed by calling function cp1_dec

				cp1_goto(1,0);						// set the position of message to be displayed
				cp1_str("Speed2:");		// send message to be displayed by calling function cp1_str
				cp1_dec(bh_2_spval(add_bh1),3);		// send decimal to be displayed by calling function cp1_dec
				cp1_str(" ");		// send message to be displayed by calling function cp1_str
				cp1_dec(bh_2_enval(add_bh1),5);		// send decimal to be displayed by calling function cp1_dec
		...
		else if(cp1_sw(3)==0)			   		// Test whether SW3 on IFC-CP04 is pressed
		{
			cp1_clr();
			temp1=0;						// clear temp1
			...
			for(i=0;i<3;i++)			// buzzer beeps and LCD displays mode for 3 time
			{
				cp1_goto(0,5);			// set the position of message to be displayed
				cp1_str("Demo 3");		// send message to be displayed by calling function cp1_str
				buzzer = 1;				// ON buzzer
				delay(100000);			// time delay for buzzer ON
				cp1_clr();				// clear LCD screen by calling function cp1_clr
				buzzer = 0;				// OFF buzzer
			...
			cp1_goto(1,11);		// set the position of message to be displayed
			cp1_str("CCW");		// send message to be displayed by calling function cp1_str
			...
			while(1)
			{
				...
				if(temp1>500)					// the value of temp1 here will determine the 
				{								// acceleration of the motors, the bigger the value here, 
					...
					else if(speed1<=0)			
					{
						...
						cp1_goto(0,11);					// set the position of message to be displayed
						dir1^=1;						// change direction of motor 1 for 	
						if(dir1==0)						// every speed 1 is equal to 0.
						{
							bh_1_ccw(add_bh1);	// Run motor 1 in counter-clockwise if dir1=0
							cp1_str("CCW");		// send message to be displayed by calling function cp1_str
						}
						else 
						{
							bh_1_cw(add_bh1);	// Run motor 1 in clockwise if dir1=1
							cp1_str("CW ");		// send message to be displayed by calling function cp1_str
						}
					}
					...
					else if(speed2<=0)
					{
						...
						if(dir2==0)						// every speed 2 is equal to 0.
						{
							bh_2_ccw(add_bh1);	// Run motor 2 in counter-clockwise if dir1=0
							cp1_str("CCW");		// send message to be displayed by calling function cp1_str
						}
						else 
						{
							bh_2_cw(add_bh1);	// Run motor 2 in clockwise if dir1=1
							cp1_str("CW ");		// send message to be displayed by calling function cp1_str
						}
					}
